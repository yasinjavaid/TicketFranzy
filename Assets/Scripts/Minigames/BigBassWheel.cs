...
public class BigBassWheel : ArcadeGame
{
    ...
    [SerializeField] private float rotationVelocityMin = 10;
    ...
    public void OnWheelTriggerEnter(Collider collider)
    {
        if (int.TryParse(collider.name.Split('_').Last(), out int value))
            currentValue = value;
    }
